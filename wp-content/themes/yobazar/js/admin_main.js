...
jQuery(function($){
	...
	$('.ts-gallery-box .add-image').on('click', function(event){
        ...
        var _states = [new wp.media.controller.Library({
            filterable: 'uploaded',
            title: yobazar_admin_texts.select_images,
            multiple: true,
            priority:  20
        })];
        ...
        file_frame = wp.media.frames.file_frame = wp.media({
            states: _states,
            button: {
                text: yobazar_admin_texts.use_images
            }
        });
...
jQuery(function($){
	...
	if( add_sidebar_form.length > 0 ){
		...
		$('#ts-add-sidebar').on('click', function(e){
			...
			if( sidebar_name != '' ){
				...
				var data = {
					action: 'yobazar_add_custom_sidebar'
	...
	if( $('.sidebar-ts-custom-sidebar').length > 0 ){
		...
		$('.sidebar-ts-custom-sidebar .delete-sidebar').on('click', function(){
			...
			var sidebar_nonce = $('#ts_custom_sidebar_nonce').val();
			var ok = confirm( yobazar_admin_texts.delete_sidebar_confirm );
			if( ok ){
				widget_block.hide();
				var data = {
					action: 'yobazar_delete_custom_sidebar'
					,sidebar_name: sidebar_name
					,sidebar_nonce: sidebar_nonce
				};
				...
				$.ajax({
					...
					,success : function(response){
						if( response != '' ){
							...
							widget_block.show();
							alert( yobazar_admin_texts.delete_sidebar_failed );
						}
					}
				});
			}
...
jQuery(function($){
	...
	$( document ).on( 'click', '.ts-product-cat-upload-field .upload-button', function( event ) {
		...
		file_frame = wp.media.frames.downloadable_file = wp.media({
			title: yobazar_admin_texts.choose_an_image,
			button: {
				text: yobazar_admin_texts.use_image
			},
			multiple: false
		});
