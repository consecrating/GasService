...
if( yobazar_is_active_filter_area() ){
	...
	$show_filter_area_by_default = yobazar_show_filter_area_by_default();
}

$extra_class = '';
$page_column_class = yobazar_page_layout_columns_class($theme_options['ts_prod_cat_layout']);
...
yobazar_breadcrumbs_title($show_breadcrumb, $show_page_title, woocommerce_page_title(false));
?>
<div class="page-container <?php echo esc_attr($extra_class) ?>">
	...
	?>
	<div id="main-content" class="<?php echo esc_attr($page_column_class['main_class']); ?> <?php echo esc_attr( $show_filter_area_by_default?'show-filter-sidebar show-filter-default':'' ) ?>">	
		...
		<?php if ( woocommerce_product_loop() ) : ?>
			...
			if( class_exists('WC_Widget_Layered_Nav_Filters') ){
				...
				the_widget('WC_Widget_Layered_Nav_Filters', array('title' => esc_html__('Active filters:', 'yobazar')));
				echo '</div>';
			}
			?>
			...
			?>
			<div class="woocommerce main-products columns-<?php echo esc_attr($woocommerce_loop['columns']); ?>">
			<?php
